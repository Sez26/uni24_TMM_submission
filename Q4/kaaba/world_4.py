...
class Pedestrian:
    def __init__(self, id, birth_time, starting_position, source, destination, radius, colour, group_name=None):
        self.id = id
        ...
        # rotations counter
        self.num_circles = 0
        self.prev_pos = starting_position
        self.complete = False
        # self.start_angle = 0
...
class World:
    ...
    def update_positions(self, dt=0.05):
        ...
        for i, ped_i in self.pedestrians.items():
            ped_i.prev_pos = np.copy(ped_i.pos)
    ...
    def birth_pedestrians(self, dt):
        for group_name, data in self.pedestrian_sources.items():
            ...
            while len(self.get_statistics(["source", group_name]))<max_count and random.random() < dt*birth_rate:
                radius = 2.2 + 0.1*random.random() # i have changed this first number from 0.4
                x0, y0 = self.get_random_position(polygon_name=data["source"], margin=radius)
                ped = Pedestrian(self.next_pedestrian_id, self.time, np.array([x0, y0]), data["source"], data["destination"], radius, data["colour"], group_name=group_name)
    ...
    def render(self):
        ...
        # Create figure and axis only once (outside the animation loop)
        if not hasattr(self, 'fig') or not hasattr(self, 'ax'):
            self.fig, self.ax = plt.subplots()

        # Optional: clear the previous content of axes if needed (do it once before animation loop)
        # self.ax.clear()

        # Update data for circle (16-point polygon)
        circle = []
        for i in range(16):
        ...
        for name, poly in self.polygons.items():
            self.ax.fill(poly["nodes"][:, 0], poly["nodes"][:, 1], facecolor=poly["colour"], edgecolor="none")

        # Plot pedestrians
        for ped in self.pedestrians.values():
            x0, y0, rad, col = ped.pos[0], ped.pos[1], ped.radius, ped.colour
            self.ax.fill(x0 + circle[:, 0] * rad, y0 + circle[:, 1] * rad, facecolor=col, edgecolor="none")

        # Plot boundaries
        for boundary in self.boundaries:
            self.ax.plot([boundary[0], boundary[2]], [boundary[1], boundary[3]], "k-", linewidth=5)

        # Update limits and labels
        self.ax.set_xlim(-50, 50)
        self.ax.set_ylim(-50, 50)
        self.ax.set_title(str(round(100 * self.time) / 100.0) + " s")
        ...
        plt.pause(0.01)  # This will allow for the figure to be updated at each frame

    
    def render_2(self, frame):
        # This will be called at each frame to update the plot
        self.update(self.render_int)
        
        # Update time for the animation
        self.time = frame  # 'frame' is automatically passed by FuncAnimation

        # Create or update figure and axis only once
        if not hasattr(self, 'fig') or not hasattr(self, 'ax'):
            self.fig, self.ax = plt.subplots()

        # Optional: Clear previous plot (if needed)
        self.ax.clear()

        # Define circle for pedestrians (16-point polygon)
        circle = []
        for i in range(16):
            circle.append([math.cos(2 * math.pi * i / 16.0), math.sin(2 * math.pi * i / 16.0)])
        circle = np.array(circle)
        ...
        self.ax.set_xlim(-50, 50)
        self.ax.set_ylim(-50, 50)
        self.ax.set_title(f"{round(100 * self.time) / 100.0} s")
        self.ax.set_xlabel("x (m)")
        self.ax.set_ylabel("y (m)")
    ...
    def save_animation(self, filename):
        # creating time span of animation
        self.sim_time = 200 # s
        self.render_int = 0.5 # s
        # Create the animation object
        ani = animation.FuncAnimation(self.fig, self.render_2, frames=np.arange(0,self.sim_time,self.render_int), interval=100, repeat=False)
