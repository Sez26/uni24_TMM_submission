...
class Simulation:
    ...
    def update(self, dt):
        ...
        if random.random()<self.inflow*dt:
            if self.traffic:
                # do nothing, no new incoming cars whilst light is red
                pass
        ...
        for i in range(n-1, -1, -1):
            ...
            v_e = self.vehicles[i].v_desired/2*(math.tanh(d_alpha - self.d_c) + math.tanh(self.d_c))
    ...
    def render(self,ax):
        ...
        ax.set_title("Model 1: " + str(round(100*self.time)/100.0) + " s")
